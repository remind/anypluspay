 ...
 * 2024/1/27
 */
public abstract class AbstractBaseProcessService {
    ...
    protected FluxOrder createAndStoreFluxOrder(BasePayProcess payOrder) {
    ...
    protected FluxOrder buildFluxOrder(BasePayProcess payOrder) {
        FluxOrder fluxOrder = new FluxOrder();
        fluxOrder.setPaymentId(payOrder.getPaymentId());
        fluxOrder.setPayProcessId(payOrder.getProcessId());
    ...
    protected PayOrderType getPayOrderType(BasePayProcess payOrder) {
        if (payOrder instanceof PayProcess) {
            return PayOrderType.PAY;
        } else if (payOrder instanceof RefundProcess) {
    ...
    private void fillFluxInstruction(FluxOrder fluxOrder, FundDetail fundDetail) {
        ...
        fluxProcess.setPaymentId(fundDetail.getPaymentId());
        fluxProcess.setPayProcessId(fundDetail.getPayProcessId());
