...
public class RefundApplyService extends AbstractPayService {
     ...
     */
    public PayResult apply(String tradeId, String payOrderId, RefundType refundType) {
        PayOrder payOrder = payOrderRepository.load(payOrderId);
        RefundOrder refundOrder = buildRefundOrder(tradeId, payOrder.getAmount(), refundType, payOrder);
     ...
     * @return 退款指令
     */
    public RefundOrder buildRefundOrder(String tradeId, Money refundAmount, RefundType refundType, PayOrder payOrder) {
        RefundOrder refundOrder = new RefundOrder();
        refundOrder.setOrderId(idGeneratorService.genIdByRelateId(tradeId, PayOrderType.REFUND.getIdType()));
     ...
     * @return 收款方明细
     */
    private List<FundDetail> buildPayeeDetails(String tradeId, String orderId, Money totalAmount, List<FundDetail> origPayerDetails, List<FundDetail> refundedPayeeDetails) {
        ...
        origPayerDetails.forEach(origPayerDetail -> {
            if (refundAbleAmount.greaterThan(new Money(0, refundAbleAmount.getCurrency()))) {
                FundDetail refundPayeeDetail = reverseFundDetail(tradeId, orderId, refundAbleAmount.clone(), origPayerDetail, refundedPayeeDetails);
     ...
     * @return 收款指令资金明细
     */
    private List<FundDetail> buildPayerDetails(String tradeId, String orderId, Money totalAmount, List<FundDetail> origPayeeDetails, List<FundDetail> refundedPayerDetails) {
        List<FundDetail> fundDetails = new ArrayList<>();
        ...
        origPayeeDetails.forEach(payerDetail -> {
            if (requestRefundAmount.greaterThan(new Money(0, requestRefundAmount.getCurrency()))) {
                FundDetail refundPayeeDetail = reverseFundDetail(tradeId, orderId, requestRefundAmount.clone(), payerDetail, refundedPayerDetails);
                if (refundPayeeDetail != null) {
                    fundDetails.add(refundPayeeDetail);
     ...
     * @return 退款指令资金明细
     */
    private FundDetail reverseFundDetail(String tradeId, String orderId, Money amount, FundDetail origFundDetail, List<FundDetail> allFundedDetails) {
        ...
        reverseFundDetail.setTradeId(tradeId);
